...
import { isPreviouslyPublished, isSameTopic } from './story-dedup.mjs'
...
const publishedHistoryPath = resolve(scriptDir, '../src/content/published-story-history.json')
...
function loadPublishedHistory() {
  try {
    const history = JSON.parse(readFileSync(publishedHistoryPath, 'utf8'))
    return Array.isArray(history.stories) ? history.stories : []
  } catch {
    return []
  }
}

function historyEntry(story) {
  return {
    headline: story.headline,
    context: story.context ?? `${story.summary ?? ''} ${story.whyItMatters ?? ''}`.trim(),
    sourceUrl: story.sourceUrl,
    publishedAt: story.publishedAt,
  }
}

function writePublishedHistory(existingHistory, currentStories, selectedStories) {
...
async function main() {
  ...
  const currentStories = loadStorySeeds()
  const publishedHistory = loadPublishedHistory()
  const previouslyPublished = [...publishedHistory, ...currentStories]
  for (const candidate of candidates.sort((a, b) => b.publishedAt - a.publishedAt)) {
    ...
    if (isPreviouslyPublished(topic, previouslyPublished)) continue
  ...
  writePublishedHistory(publishedHistory, currentStories, selected)
